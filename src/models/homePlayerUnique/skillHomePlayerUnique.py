...
for index, row in df.iterrows():
    ...
    if team1 == row['team1']:
        if row['porcentage_attendance']>=0.75:
            composition.append([[row['team1'], 'localia_3'], [row['team2']]])
        elif (row['porcentage_attendance']>=0.0) &(row['porcentage_attendance']<0.75):
            composition.append([[row['team1'], 'localia_2'], [row['team2']]])
        else:
            composition.append([[row['team1'], 'localia_1'], [row['team2']]])
    else:
        composition.append([[row['team1']], [row['team2']]])

print("End composition appending")
with open(root / 'models/parameters.txt') as f:
    lines = f.readlines()
    for i in lines:
        variable = i.split(':')
        if variable[0] == "Gamma":
...
prior_dict["localia_1"] = ttt.Player(ttt.Gaussian(2., sigma), beta, gamma)
prior_dict["localia_2"] = ttt.Player(ttt.Gaussian(2., sigma), beta, gamma)
prior_dict["localia_3"] = ttt.Player(ttt.Gaussian(2., sigma), beta, gamma)
